...
func TestSimulateMsgSubmitProposal(t *testing.T) {
	...
	require.NotNil(t, msg.GetContent())
	...
	require.NoError(t, err)
	require.Equal(t, []sdk.Msg{types.NewMsgVote(app.GovKeeper.GetGovernanceAccount(ctx).GetAddress(), 0, types.OptionYes)}, msgs)
...
func TestSimulateMsgDeposit(t *testing.T) {
	...
	proposal, err := types.NewProposal(content, []sdk.Msg{}, 1, submitTime, submitTime.Add(depositPeriod))
	require.NoError(t, err)
