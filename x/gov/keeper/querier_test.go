...
func TestQueries(t *testing.T) {
	...
	proposal1, err := app.GovKeeper.SubmitProposal(ctx, tp, []sdk.Msg{})
	require.NoError(t, err)
	...
	proposal2, err := app.GovKeeper.SubmitProposal(ctx, tp, []sdk.Msg{})
	require.NoError(t, err)
	...
	proposal3, err := app.GovKeeper.SubmitProposal(ctx, tp, []sdk.Msg{})
	require.NoError(t, err)
